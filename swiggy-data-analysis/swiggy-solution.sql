...
with rankbycat as (select restaurant_name
,count(distinct menu_category) as cnt_cat
,dense_rank() over(order by count(distinct menu_category) desc) as drnk
from swiggy
group by restaurant_name)

select * from rankbycat
where drnk<=5

14. Which restaurant provides the highest percentage of non-vegetarian food?

with non_vegeterian_data as (select restaurant_name
,count(case when veg_or_nonveg='Non-veg' then 1 end)*100/count(*) as Nonveg_percetage
from swiggy
group by restaurant_name)

,rankbyhighperc as (
select *
,rank() over(order by Nonveg_percetage desc) as rnk
from non_vegeterian_data
)

select * from rankbyhighperc
where rnk=1
